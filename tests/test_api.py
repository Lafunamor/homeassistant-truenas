...
def connect_factory_fixture(monkeypatch):
    ...
    def _install(responses: list):
        ...
        monkeypatch.setattr("custom_components.truenas.api.connect", _connect)
        return created
...
#   query
# ---------------------------
def test_query_returns_result(connect_factory) -> None:
    ...
    assert api.query("system.info") == {"version": "25.10"}
    assert api.error == ""


def test_query_ignores_unrelated_messages(connect_factory) -> None:
    ...
    api = TrueNASAPI("10.0.0.1", "key", False)
    assert api.connect() is True
    assert api.query("pool.query") == ["expected"]


def test_query_error_response_returns_none(connect_factory) -> None:
    ...
    api = TrueNASAPI("10.0.0.1", "key", False)
    assert api.connect() is True
    assert api.query("reporting.get_data") is None
    assert api.error == "ENOMETHOD"


def test_query_error_keeps_connection(connect_factory) -> None:
    ...
    api = TrueNASAPI("10.0.0.1", "key", False)
    api.connect()
    assert api.query("broken.method") is None
    assert api.connected() is True
    assert api.query("system.info") == "second call"
    assert len(created) == 1


def test_query_transport_error_disconnects(connect_factory) -> None:
    ...
    api = TrueNASAPI("10.0.0.1", "key", False)
    api.connect()
    assert api.query("system.info") is None
    assert api.connected() is False
    assert api.error == "cannot_connect"
    assert created[0].closed is True


def test_invalid_api_key(connect_factory) -> None:
...
def test_disconnect_closes_and_clears(connect_factory) -> None:
...
def test_query_params_are_wrapped(connect_factory) -> None:
    ...
    api.connect()
    api.query("no.params")
    api.query("dict.params", {"a": 1})
    api.query("list.params", [1, 2])
    sent = created[0].sent
...
def test_request_ids_are_unique(connect_factory) -> None:
